        ...
        if op.type == 26:
            ...
            if 'MENTION' in msg.contentMetadata.keys() != None:
                 if wait["kickMention"] == True:
                     ...
                     for mention in mentionees:
                           if mention['M'] in mid:
                                  acil.sendText(msg.to,"Jangan tag gw gw gw lagi sensi😡😡😡")
...
#------------------------------------------------------------
            if msg.text in ["Invite:user"]:
              if msg.from_ in PRANKBOT:
                wait["invite"] = True
                acil.sendText(msg.to,"kirim kontak tikungannya?")
#------------------------------------------------------------
	    ...
	    if "Xvideos " in msg.text:
            ...
            if "Help" == msg.text:
              if msg.from_ in PRANKBOT:
                if wait["lang"] == "JP":
                	...
                	acil.sendText(msg.to,helpMessage1)
                	msg.contentType = 13
                	acil.sendText(msg.to,"ADD CREATOR >Silent bot line.me/ti/p/~dhenz415")
                	msg.contentMetadata = {'mid':'ufdc20b3a00b5e8f31e4f91017eb361b0'}
                	acil.sendMessage(msg)
            if "Media" == msg.text:
              if msg.from_ in PRANKBOT:
            ...
            if "Respon" == msg.text:
              if msg.from_ in PRANKBOT:
                ...
                ki5.sendMessage(msg)
                ki.sendText(msg.to,"ADD CREATOR >Silent bot line.me/ti/p/~dhenz415")
            if "Set" == msg.text:
              if msg.from_ in PRANKBOT:
                md = "╔▬▬SILENT BOT▬▬╗\n║▬▬▬S▬▬K▬▬▬║\n"
...
#======ADMIN======
            ...
            if "Respon" == msg.text:
              if msg.from_ in wait["ACIL"]:
                ...
                msg.contentMetadata = {'mid': ki5mid}
                ki5.sendMessage(msg)
                ki.sendText(msg.to,"HALLO SILENT ON KMBALI SMUA SIAP PROTECT")
            if "Getvid @" in msg.text:
              if msg.from_ in wait["ACIL"]:
                print "[Command]dp executing"
                _name = msg.text.replace("Getvid @","")
            ...
            if msg.text in ["Invite:user"]:
              if msg.from_ in wait["ACIL"]:
                wait["invite"] = True
                acil.sendText(msg.to,"Kirim no tikungan nya😜😜😜")
            if "Google " in msg.text:
              if msg.from_ in wait["ACIL"]:
                    a = msg.text.replace("Google ","")
                    b = urllib.quote(a)
...
#----------------------------------------------
		    ...
		    a = msg.text.replace("Xvideos ","")
                ...
                msg.contentMetadata = {'mid': ki5mid}
                ki5.sendMessage(msg)
                ki.sendText(msg.to,"🔥🔥🛠SILENT HADIR SMUA BOS SIAP PROTECT🛠🔥🔥")
            elif "B1" == msg.text:
                msg.contentType = 13
                msg.contentMetadata = {'mid': kimid}
                ki.sendMessage(msg)
...
#	      if msg.from_ in admin:
		       ...
		       acil.sendText(msg.to,"Wating in progres...")
                       ...
                       acil.sendText(msg.to, "Succes ALL bos dhenza...!!!!")
                       print " Spammed !"
#--------------------------------------------------------------------------
#-----------------------------------------------------------
...
#-----------------------------------------------
 ...
 #---------Fungsi Banlist With Tag--------#
                ...
                if msg.toType == 2:
                   ...
                   ki3.sendText(msg.to,"ok🖒")
                   ki4.sendText(msg.to,"ok🖒")
                   ki5.sendText(msg.to,"ok 🖒")
                   ki.sendText(msg.to,"《▪●Dendam sudah terbalaskan●▪》")
...
#------------------------------------------------------------
                ...
                wait["invite"] = True
                random.choice(KAC).sendText(msg.to,"Send kontak tikunganya 😆😆😆")
#------------------------------------------------------------
                ...
                for j in xrange(k+1):
                    ...
                    s=0
                    d=[]
                    for i in group.members[j*500 : (j+1)*20]:
                        d.append({"S":str(s), "E" :str(s+8), "M":i.mid})
                        s += 9
                        txt += u'@Krampus\n'
                    msg.text = txt
